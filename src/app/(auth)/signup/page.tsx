...
const PLAN_LABELS: Record<string, { label: string; price: string; color: string }> = {
  ...
  enterprise: { label: 'Enterprise Plan', price: '$199/mo', color: 'text-amber-400 border-amber-500/30 bg-amber-500/10' },
};
...
function SignupForm() {
  ...
  const handleSignup = async (e: React.FormEvent) => {
    ...
    if (signupError) {
      ...
      // Store plan in sessionStorage so post-verification redirect can use it
...
export default function SignupPage() {
  return (
    <Suspense fallback={<div className="w-full max-w-md h-96 bg-white/5 rounded-2xl animate-pulse" />}>
      <SignupForm />
    </Suspense>
  );
}
